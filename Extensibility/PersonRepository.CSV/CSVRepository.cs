...
namespace PersonRepository.CSV
{
    public class CSVRepository : IPersonRepository
    {
        ...
        {
            //Text file has to be in same folder as executable because of same path.
        ...
        public IEnumerable<Person> GetPeople()
        {
            ...
            if (File.Exists(path))
            {
                // reading each line with StreamReader
                var sr = new StreamReader(path);
                string line;

                while ((line = sr.ReadLine()) != null)
                {
                    //parse line based on commas as separators
                    var elems = line.Split(',');
                    // turn into person objects
                    var per = new Person()
                    {
                        FirstName = elems[0],
                        LastName = elems[1],
